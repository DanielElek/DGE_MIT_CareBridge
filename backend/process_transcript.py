...
tokenizer_lora = None
model_vanilla = None
tokenizer_vanilla = None

import threading
_model_lock = threading.Lock()

def preload_models():
    """Load the ML models into memory to accelerate generation."""
    global model_lora, tokenizer_lora, model_vanilla, tokenizer_vanilla
    
    with _model_lock:
        if model_lora is not None and model_vanilla is not None:
            print("SOAP models already loaded.")
            return

        if model_lora is None:
            print(f"Preloading LoRA model from {MODEL_PATH_LORA}...")
            model_lora, tokenizer_lora = load(MODEL_PATH_LORA)
            
        if model_vanilla is None:
            print(f"Preloading Vanilla model from {MODEL_PATH_VANILLA}...")
...
def prune_soap(model, tokenizer, transcript: str, parsed_sections: dict) -> tuple[dict, dict]:
    # Build final validated SOAP note dictionary and sentence→quote mappings
    pruned_sections = {k: "Not discussed." for k in SOAP_KEYS}
    quotes_map = {k: [] for k in SOAP_KEYS}
    ...
    for key in SOAP_KEYS:
        ...
        for sent in sentences:
            if not is_substantive(sent):
                ...
                quotes_map[key].append({"sentence": sent, "quote": ""})
            ...
            if verdict == "YES":
                valid_sentences.append(sent)
                # Store the exact transcript quote that supports this sentence
                quote_text = evidence if evidence and evidence != "Verified" else ""
                quotes_map[key].append({"sentence": sent, "quote": quote_text})
    ...
    return pruned_sections, quotes_map

def run_process_transcript(input_txt="formatted_dialogue.txt", output_json_filename="final_soap_summary.json"):
    """
    Runs the SOAP parsing and generating pipeline. Assumes `preload_models()` called previously.
    Yields JSON string statuses.
    """
    global model_lora, tokenizer_lora, model_vanilla, tokenizer_vanilla

    if model_lora is None or model_vanilla is None:
        yield format_yield("info", "Models not preloaded. Loading now...")
        preload_models()

    yield format_yield("info", f"Reading transcript from {input_txt}...")
    try:
        with open(input_txt, "r", encoding="utf-8") as file:
            transcript = file.read()
    except FileNotFoundError:
        yield format_yield("error", f"Could not find {input_txt}.")
        return

    # Start total timer
    start_time_total = time.perf_counter()

    yield format_yield("info", "STEP 1: Generating Initial SOAP Note...")
    
    # Start generation timer
    start_time_gen = time.perf_counter()
    prompt = PROMPT_TEMPLATE.format(transcript)
    
    # Generate SOAP (takes time)
    yield format_yield("info", "Executing LoRA model on transcript...")
    ...
    yield format_yield("info", f"STEP 2: Validating Generated Content ({time_gen:.2f}s)...")
    parsed_soap = repair_and_parse_json(raw_response)
    
    if parsed_soap is None:
        yield format_yield("error", "Could not parse JSON output. Halting pruning.")
        return

    warnings_list = validate_soap(parsed_soap, transcript)
    if warnings_list:
        for w in warnings_list:
            yield format_yield("warn", f"Validation warning: {w}")
    else:
        yield format_yield("info", "Validation passed with no preliminary warnings.")

    yield format_yield("info", "STEP 3: Per-Sentence Verification and Pruning...")
    
    # Start pruning timer
    start_time_prune = time.perf_counter()
    # verify_sentence will print its own logs, but won't ruin SSE streams unless we capture output
    # (FastAPI doesn't capture stdout by default, our thread wrapper will handle generator)
    final_soap_dict, quotes_map = prune_soap(model_vanilla, tokenizer_vanilla, transcript, parsed_soap)
    # End pruning timer
    end_time_prune = time.perf_counter()
    time_prune = end_time_prune - start_time_prune

    # Write Final Output as JSON (includes both soap and quotes)
    final_output = {"soap": final_soap_dict, "quotes": quotes_map}
    final_soap_json = json.dumps(final_output, indent=4)
    ...
    yield format_yield("info", f"Processing Complete! Execution: {time_total:.2f}s")
    yield json.dumps({"status": "returncode", "code": 0}) + "\n"

if __name__ == "__main__":
    for text in run_process_transcript():
        print(text, end="")